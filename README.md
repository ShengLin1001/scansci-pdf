...
- **Cloudflare 绕过** — CloakBrowser 反检测浏览器（原生 Chromium stealth 引擎，内置指纹修补，无需 Docker）+ curl_cffi TLS 指纹模拟
...
Tier 4 (25s) ─ LibGen / Sci-Hub（带 CloakBrowser 反检测绕过）
...
1. **Phase 1 — 免费源并行竞速**（15s 超时）：出版商直连、OA API（Unpaywall/OpenAlex/SemanticScholar/DOAJ/EuropePMC/CORE/PMC）、Sci-Hub/LibGen、浏览器源
2. **Phase 2 — 机构访问**（30s 超时）：仅当 Phase 1 全部失败时触发，通过 instsci 桥接、CARSI、WebVPN、EZProxy 获取
...
当 Sci-Hub、LibGen 等站点触发 Cloudflare 防护时，CloakBrowser 可以自动绕过。CloakBrowser 是原生 Chromium stealth 浏览器引擎，内置指纹修补，无需 Docker。如果已安装 curl_cffi，会优先使用 TLS 指纹模拟（更快）。

---

## 故障排查

**Sci-Hub 下载失败** — 运行 `scansci_pdf_health_check(detailed=true)` 查看数据源状态。域名轮换自动处理。如果遇到 Cloudflare 防护，确保 CloakBrowser 服务运行或安装 curl_cffi。
...
**下载速度慢** — 运行 `scansci_pdf_health_check(detailed=true)` 检查数据源延迟。如 Sci-Hub 在你的网络被封锁，配置代理或禁用 Sci-Hub（`scansci_pdf_config_set scihub_enabled false`）。
