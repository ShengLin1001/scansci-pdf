...
def candidate_profile_dirs(config: dict | None = None, *, workspace: Path | None = None) -> list[Path]:
    """Return known local browser profile candidates in priority order."""
    config = config or {}
    candidates = [
        Path(config["chrome_profile_dir"]) if config.get("chrome_profile_dir") else None,
        DATA_DIR / "browser_profiles" / "webvpn",
        DATA_DIR / "chrome-profile",
    ]
